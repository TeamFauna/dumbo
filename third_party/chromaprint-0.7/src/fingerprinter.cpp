...
Fingerprinter::Fingerprinter(FingerprinterConfiguration *config)
    ...
    m_overlap(m_frame_size - m_frame_size / 3)
{
	if (!config) {
	...
	m_chroma = new Chroma(MIN_FREQ, MAX_FREQ, m_frame_size, SAMPLE_RATE, m_chroma_filter);
	//m_chroma->set_interpolate(true);
	m_fft = new FFT(m_frame_size, m_overlap, m_chroma);
...
void Fingerprinter::ResetFrameSize(int frame_size) {
  m_frame_size = frame_size;
  m_overlap = m_frame_size - m_frame_size / 3;
  ...
  if (m_silence_remover) {
    m_silence_remover->set_consumer(m_fft);
  } else {
    m_audio_processor->set_consumer(m_fft);
  }
}
